...
| `.agents/skills/` | Managed agents bridge skills for OpenAI Codex CLI and GitHub Copilot CLI (generated by installer, not repo-owned) |
...
- `install.py` — Core installer (`SkillInstaller`, `SkillSelector`); handles skill copying, agents bridge generation, Claude reference generation, `.gitignore` management, and `post_install.py` hooks
- `install.config.json` — Machine-readable skill catalog; controls `distribution` (`"shared"` | `"gemini-only"`) and `supports.agents_bridge` / `supports.claude_reference`
...
| Install / update / agents bridge setup | `skill-manager` skill |
...
- `distribution: "shared"` → must support an agents bridge (`.agents/skills/`) and a Claude reference skill (`.claude/skills/`) via `skill-manager`.
- `distribution: "gemini-only"` → no agents bridge or Claude reference. The `subagent-balancer` family is the canonical example.

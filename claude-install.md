# Claude Skill Installation Instructions

This file provides the necessary context for Claude Code to install official skills from this repository into another project.
...
2.  **Initialize the Installer**:
    ...
    For Claude-driven install logic, use the `SkillInstaller` and `SkillSelector` classes from `./install.py` in this repository.
...
3.  **Process**:
    ...
    -   Prompt the user (via AskUserQuestion) to select the skills they want to install or update.
    -   Perform the installation/update using `installer.install_skill(skill_path, target_project_path)`. Skills install into `.claude/skills/`.

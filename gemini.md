...
- Every skill marked `distribution: "shared"` must be installable with both a Codex bridge and a Claude reference skill through `skill-manager`.
- Treat `.codex/skills/` and `.claude/skills/` entries created by `skill-manager` as managed installation artifacts, not as repo-owned source files.
...
- When adding a new shared skill or changing bridge eligibility, update installer behavior and tests so both tool-specific artifacts are generated together.

## Documentation Sync

- `AGENTS.md`, `gemini.md`, and `claude.md` are policy mirrors and must stay synchronized.
- When updating one of these files, merge the change into the other two in the same task.
...
- After publishing, verify any related Codex bridge wrappers in `.codex/skills/` and Claude reference skills in `.claude/skills/` still accurately describe the Gemini skill they point to.

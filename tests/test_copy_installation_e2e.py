...
class TestCopyInstallationE2E(unittest.TestCase):
    ...
    def test_install_skill_copies_files(self) -> None:
        """Verify that install_skill physically copies files to .claude/skills/."""
        skill_rel_path = f"{self.skill_cat}/{self.skill_name}"
        success = self.installer.install_skill(skill_rel_path, self.project_dir)

        self.assertTrue(success)

        target_path = os.path.join(self.project_dir, ".claude", "skills", self.skill_name)
        ...
        self.assertNotIn(".claude/skills/\n", gitignore)
    ...
    def test_install_skill_overwrites_existing(self) -> None:
        ...
        target_skill_md = os.path.join(self.project_dir, ".claude", "skills", self.skill_name, "SKILL.md")
    ...
    def test_uninstall_skill_removes_managed_artifacts(self) -> None:
        """Verify uninstall removes the Claude skill and refreshes gitignore."""
        skill_rel_path = f"{self.skill_cat}/{self.skill_name}"
        self.installer.install_skill(skill_rel_path, self.project_dir)
    ...
    def test_uninstall_skill_ignores_unmanaged_skill(self) -> None:
        ...
        unmanaged_path = os.path.join(self.project_dir, ".claude", "skills", "local-only")
    ...
    def test_uninstall_skill_returns_false_when_a_managed_artifact_is_locked(self) -> None:
        ...
        claude_path = os.path.join(self.project_dir, ".claude", "skills", self.skill_name)
        ...
        self.assertFalse(success)
        self.assertTrue(os.path.isdir(claude_path))
    ...
    def test_transition_from_junction_to_copy(self) -> None:
        ...
        skill_rel_path = f"{self.skill_cat}/{self.skill_name}"
        target_skills_dir = os.path.join(self.project_dir, ".claude", "skills")
        ...
        self.assertTrue(os.path.islink(target_path) or bool(attrs & 0x400))
